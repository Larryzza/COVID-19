library(ggplot2)
setwd("D:/RA-POLYU/wuhan1.23/submit")#setwd()
##### Tencent
...
#resul.baidu<-data.frame(resul,type="Baidu")
#resul.tencent<-data.frame(resul,type="Tencent")
#dev.off()
#par(mfrow=c(1,2))
#par(las=1)
...
resul.com<-rbind(resul.baidu,resul.tencent)
colnames(resul.com)[3]<-"source"
ggplot ()+
  geom_line(data = resul.com, aes(x=a, y=V1,group=source,color=source))+
  scale_color_manual(values = c("darkred", "darkblue"))+
  labs(x= "# of COVID-19 cases (λ)",y = "log-likelihood")+ #,title = "effectiveness of different actions"
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5,size = 20, face = "bold"),
        axis.text=element_text(size=21),axis.title.x=element_text(size=21),
        axis.title.y=element_text(size=21),legend.text=element_text(size=13),legend.title = element_text(size=13),
...
TD<-4924 
BD<-13592
round(0.514*TD+0.486*BD)

######scaling test
'
b.pop<-data[,c(1,3)]
t.pop<-data[,c(1,3)]
colnames(b.pop)[1]<-"place"
colnames(t.pop)[1]<-"place"
pop.com<-plyr::join(t.pop,b.pop,type="left",by="place")
...
#pop.add<-pop.add[c(1:24),-4]
ggplot ()+
  geom_point(data = pop.add, aes(x=pop.baidu, y=pop.tencent)) +
  #facet_wrap(~situation,scales="free")+
  labs(x= "# of population flow (baidu)",y = "# of population flow (tencent)")+ #,title = "effectiveness of different actions"
  scale_x_log10(limits = c(50, 500000))+
  scale_y_log10(limits = c(50, 500000))+
  geom_abline(intercept = 0, slope = 1,linetype="dashed",color="black")+
  theme_bw()+
